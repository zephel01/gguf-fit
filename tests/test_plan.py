...
def test_overhead_matches_the_single_measurement():
    ...
    assert 23.5 - bare <= gguf_plan.DEFAULT_OVERHEAD_GIB
...
def test_thin_headroom_is_warned_in_the_output():
    ...
    out = emit(Q5_K_M, ctx=69632, kv="f16")
    assert "headroom" in out
    assert "Use q8_0" in out             # f16 なら q8_0 を勧める


def test_thin_headroom_does_not_suggest_q8_when_already_q8():
    out = emit(Q5_K_M, ctx=131072, kv="q8_0")
    assert "Use q8_0" not in out         # 既に q8_0 なのに勧めては意味がない
    assert "Drop ctx one step" in out


def test_comfortable_headroom_is_reported_plainly():
    out = emit(Q5_K_M, ctx=32768, kv="f16")
    # "only" だけで判定すると "hybrid attention: only 17/65 layers" に当たる。
    # 警告文の固有部分で見る。
    assert "single calibration point" not in out
    assert "headroom 2.41 GiB" in out
...
def emit(rec, ctx=65536, kv="f16", lang="en"):
...
def test_emitted_command_is_valid_shell():
    import subprocess  # noqa: PLC0415 - このテストでしか使わない
...
def test_draft_mtp_omitted_when_no_mtp_tensors():
    ...
    assert "not adding" in out        # 理由は注記として残す
...
def test_japanese_output_is_available_and_differs():
    """--lang ja で日本語になる。既定は英語のまま."""
    en = emit(Q5_K_M, lang="en")
    ja = emit(Q5_K_M, lang="ja")
    assert en != ja
    assert "estimate:" in en and "見積り:" in ja
    # 機械が読む部分は翻訳しない
    for out in (en, ja):
        assert "--ctx-size 65536" in out
...
def test_max_tokens_reason_is_honest_about_which_limit_applied():
    ...
    """
    mt, key, kw = gguf_plan.max_tokens_for(131072)
    assert mt == gguf_plan.MAX_TOKENS_CAP
    assert key == "cfg_max_tokens_cap"
    assert kw["frac"] == 98304         # 3/4 が実際いくつだったかも渡す

    mt, key, kw = gguf_plan.max_tokens_for(32768)
    assert mt == 24576                 # こちらは 3/4 が効いている
    assert key == "cfg_max_tokens_frac"

    # 出力文でも言い分けられていること (両言語)
    for lang, capped, frac in (("en", "capped at", "3/4 of ctx"),
                               ("ja", "頭打ち", "の 3/4")):
        assert capped in emit(Q5_K_M, ctx=131072, lang=lang)
        assert frac in emit(Q5_K_M, ctx=32768, lang=lang)


def test_max_tokens_reason_appears_in_the_emitted_config():
    out = emit(Q5_K_M, ctx=131072, kv="q8_0")
    assert "max_tokens: 49152" in out
    assert "3/4 of ctx 131,072" not in out      # ← これが出ていたら嘘
