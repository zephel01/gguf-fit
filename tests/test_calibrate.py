...
def test_warmup_posts_a_completion_request(monkeypatch):
    sent = {}
...
def test_warming_up_moves_the_intercept_and_not_the_slope():
    """**見積り式の形が正しいことの裏付け。**

    4点とも +100 MiB 動いたのに、傾きは 1 バイトも変わらず切片だけが
    0.098 GiB (= 100 MiB) 上がった。推論が確保する分は ctx に比例しない
    定数なので、こうなるのが正しい。逆に傾きが動いたら、測り方か式の
    どちらかが間違っている。
    """
...
def test_the_ratio_survives_warming_up():
    """0.624 は測るタイミングで動かない。ここが動いたら測り方を疑う."""
    cold = cal.fit_points(Q8).bytes_per_token / cal.fit_points(F16).bytes_per_token
    warm = (cal.fit_points(Q8_WARM).bytes_per_token
            / cal.fit_points(F16_WARM).bytes_per_token)
    assert warm == pytest.approx(cold, abs=1e-9)
    assert warm == pytest.approx(0.624, abs=0.005)
...
def test_the_prompt_is_sent_as_token_ids_so_the_length_is_exact():
    """文字列だと何トークンになるか語彙次第。**バッチを埋めたいなら配列**."""
    first = cal.warmup_payloads(2048)[0]
    assert isinstance(first["prompt"], list)
    assert len(first["prompt"]) == 2048


def test_a_string_prompt_is_kept_as_a_fallback():
    """トークン配列を受けないサーバのため。長さは概算になる."""
    second = cal.warmup_payloads(2048)[1]
    assert isinstance(second["prompt"], str)
    assert len(second["prompt"]) > 1000


def test_the_warmup_fills_one_batch_by_default():
    """既定 2048 = llama-server の既定 --batch-size。"""
    assert cal.WARMUP_PROMPT_TOKENS == 2048
