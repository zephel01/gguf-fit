...
# ⚠️ 余りが 0.22 GiB しかありません。実測では推論を通すとさらに 0.10〜0.14 GiB 増えたので …
...
| 切片 | **19.14 GiB** | ファイルは 18.47 | → オーバーヘッド **0.67** |

4点の最大誤差は **0 MiB**。既定のオーバーヘッドは **1.0 GiB**。本番の負荷で見えた
さらに 0.04 GiB 分も、ここに含めて安全側に取ってあります。
...
同じサーバ・同じ ctx 65,536 で、3通りの読み方をすると（待機時からの増分）:

| | MiB |
| :-- | --: |
| ロード直後 | 23,922 |
| 8トークンのリクエストを1回通したあと | 24,022（**+100**） |
| 本番のベンチマークを回している最中 | 24,064（**+142**、±50 で揺れる） |

**+100 も +142 も、f16 と q8_0 でまったく同じ値**でした。どちらも KV の量に比例しないので、
これは**推論そのものが確保する分**です。
...
4点とも +100 MiB ちょうど動き、**傾きは1バイトも変わらず**、切片だけが 19.04 → 19.14 GiB に
上がりました。定数は切片に乗るべきもので、実際にそこに乗りました。

いまの `gguf-calibrate` はサーバ自身の `/health` を待ち、バッチ1つ分のプロンプトを持つ
リクエストを1回通してから測り、**その1回で何 MiB 増えたか**を出力に書きます。短い
リクエストでは +142 のうち +100 までしか出ません。残る約 42 MiB があるので、既定の
`overhead` は実測値ぴったりではなく余裕を持たせてあります。
