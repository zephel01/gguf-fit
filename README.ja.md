...
* **なぜ代表1本で足りるのか。** 量子化が変えるのはテンソルの**型**であって
  **名前**ではありません。だから同じリポジトリの量子化違いは層構造が同じで、
  ...
  （そうと分かるように書きます）。
* **1本目のシャードに重みが無い分割モデル。** メタデータと tokenizer だけを1本目に
  置き、テンソルは2本目から始めるリポジトリがあります
  （`unsloth/Qwen3.8-Flash-Next-GGUF` の1本目は 10.9 MB でテンソル0本）。KV層は
  **テンソルを数えて**出すもので `block_count` で代用してはいけないので、1本では
  足りません。残りのシャードのヘッダも読んで束ねます。そうしたことは出力に書き、
  増えた転送量も報告に乗ります。
...
* **量子化名のフォルダには本体が入っています。** 新しい unsloth のリポジトリは
  `UD-IQ1_S/`・`Q4_K_M/`・`BF16/` … と量子化ごとにフォルダを切り、ルートには README
  しか置きません（`unsloth/Qwen3.8-Flash-Next-GGUF` はシャード3本とも `UD-IQ1_S/`
  の中）。これは候補にします。ラベルはフォルダ名なので `--pick UD-IQ1_S` で当たります。
* **それ以外のサブディレクトリの GGUF は候補にしません**が、**要らないわけでも
  ありません。** `MTP/`・`imatrix/`・`original/` に入っているものは本体ではないので
  「載るか」の判定には出しません
  （代表に選ぶと KV の単価が 17倍ずれます → [そのままにしたバグ](#簡略化する前に)）。
  量子化フォルダの中にある `mtp`/`draft` ファイルも同じ扱いです。
  ただし draft/MTP は本体と**組にして**使うもので、本体だけ落としても始まりません。

  ```bash
  gguf-fetch <repo> --fit --extras mtp    # draft/MTP だけ一緒に
  gguf-fetch <repo> --fit --extras all    # サブディレクトリのもの全部
  gguf-fetch <repo> --pick mtp            # 名指しで1本だけ
  ```

  既定が `none` なのは、**そのファイルが何に使うものかはこちらには分からない**
  からです。あることは必ず出力に書くので、要ると分かっている人が指定してください。
...
7. **リポジトリの GGUF が全部「本体」ではない。** unsloth/Qwen3.8-27B-GGUF には
   ...
   **一番小さいので代表に選ばれ**、その 4.0 KB/token (KV層 1/65) を全行に当てて
   ...
   → 代表は大きいほうから試し、`n_tensors >= block_count` を満たさないものは
   代表にしません
8. **…そして「ルートに無い＝本体でない」ではなかった。** 7 の直し方は
   「ルート直下だけを候補にする」で、これが乱暴すぎました。
   `unsloth/Qwen3.8-Flash-Next-GGUF` はシャード3本を丸ごと `UD-IQ1_S/` に入れ、
   ルートには README しか置いていません。候補が全部消えて
   **「このリポジトリに GGUF がありません」**で止まる — ファイルは目の前にあるのに、
   ダウンロードできない状態でした。
   → **フォルダ名そのものが量子化ラベルとして読めるなら本体**とみなします
   （`UD-IQ1_S`・`Q4_K_M`・`BF16`）。`MTP`・`imatrix`・`original` は量子化名として
   読めないので従来どおり除外。bpw と `n_tensors` の網はその上でそのまま効きます
9. **較正値はモデル固有なのに設定ファイルは全モデルに効く。** Qwen3.8-27B で測った
...
10. **1本目のシャードに重みが入っているとは限らない。**
   `unsloth/Qwen3.8-Flash-Next-GGUF` の `-00001-of-00003` は 10.9 MB ありますが、
   中身はメタデータと tokenizer だけで**テンソルは0本**。重みは2本目から始まります。
   「ヘッダを1本読む」＝1本目だけ読む、だったので、アーキテクチャは取れても
   数えるものが無く、`n_tensors >= block_count` に弾かれて表がサイズ判定に縮退
   しました（bpw も最大 ctx も出ない）。おまけに「どのファイルから読んだか」の行が
   **ファイル名が空欄のまま**出ていました。
   ここで `block_count` で代用したくなりますが、それはバグ 1 の再発です。この
   モデルは 48層のうち `attn_k`/`attn_v` を持つのは **12層だけ**（残り36層は
   linear attention）で、代用すると KV/token が **4倍**に出ます。
   → 1本目にテンソルが無ければ**残りのシャードのヘッダも読んで束ねます**
   （10.4 MiB → 34.4 MiB。落とすのは 67.56 GiB なので誤差）。読めたヘッダが
   1つも無いときは「ヘッダから読んだ」と書かないようにしました
