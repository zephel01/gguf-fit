...
# ⚠️ 余りが 0.22 GiB しかありません。実測では推論開始後にさらに 0.14 GiB 増えたので …
...
| 切片 | 19.02 GiB | ファイルは 18.47 | → オーバーヘッド **0.55** |
| 推論を1回通すと | +0.14 GiB | | → オーバーヘッド **0.69** |

4点の最大誤差は **0 MiB**。既定のオーバーヘッドは安全側に **1.0 GiB** に丸めてあります。
...
同じサーバ・同じ ctx 65,536 で、2つの読み方をすると:

| | MiB |
| :-- | --: |
| ロード直後 | 23,922 |
| 推論を回している最中 | 24,068（24,013〜24,115 で揺れる） |
| 差 | **+142** |

この差は **f16 でも q8_0 でもきっかり同じ 142 MiB** でした。KV の量に比例しないので、
これは**推論そのものが確保する分**です。

まずいのは2つを混ぜること。ctx 32,768 をロード直後に、ctx 65,536 を推論中に測って傾きを
出すと、定数が傾きに化けて **73.5 KB/token** になります。条件をそろえると **69.1** です。
このプロジェクトは実際にこれで間違えました。いまの `gguf-calibrate` はサーバ自身の
`/health` を待ち、短いリクエストを1回投げてから測り、**その1回で何 MiB 増えたか**を
出力に書きます。
...
ctx ごとに `llama-server` を起動し、`/health` を待ち、8トークンのリクエストを1回投げ、
`nvidia-smi` で増分を読み、サーバを落とします。4点で数分。ベンチマークは回しません。
